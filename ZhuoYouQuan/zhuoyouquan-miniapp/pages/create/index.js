...
Page({
  data: {
    ...
    editing: false,
    submitting: false,
    form: { title: "", gameTypeId: null, customGameType: "", date: tomorrow(), time: "19:30", maxPeople: "6", city: "", storeName: "", address: "", longitude: null, latitude: null, fee: "0", aa: true, newbieFriendly: true, description: "" }
  },
  onLoad() { api.gameTypes().then((types) => { const options = [...types, { id: "custom", name: "\u81ea\u5b9a\u4e49\u7c7b\u578b" }]; this.setData({ types: options }); this.applyEditDraft(options); }).catch((e) => wx.showToast({ title: e.message, icon: "none" })); },
  onShow() {
  ...
  applyEditDraft(types) {
    ...
    const form = { title: draft.title || "", gameTypeId: typeIndex >= 0 ? types[typeIndex].id : null, customGameType: "", date: start[0] || tomorrow(), time: (start[1] || "19:30").slice(0, 5), maxPeople: String(draft.maxPeople || 6), city: draft.city || "", storeName: draft.storeName || "", address: draft.address || "", longitude: draft.longitude || null, latitude: draft.latitude || null, fee: String(draft.fee || 0), aa: draft.aa !== false, newbieFriendly: draft.newbieFriendly !== false, description: draft.description || "" };
    this.setData({ editing: true, form, typeIndex, hasType: typeIndex >= 0, typeLabel: typeIndex >= 0 ? types[typeIndex].name : "请选择游戏类型", cityLabel: draft.city || "请选择城市" }, () => {
  ...
  formSnapshot() {
    ...
    return JSON.stringify({ ...form, gameTypeId: form.gameTypeId || null, customGameType: (form.customGameType || "").trim() });
  },
  ...
  changeType(e) {
    ...
    const isCustomType = type.id === "custom";
    this.setData({ typeIndex, hasType: true, isCustomType, typeLabel: type.name, "form.gameTypeId": isCustomType ? null : type.id, "form.customGameType": isCustomType ? this.data.form.customGameType : "" });
  },
  chooseLocation() {
  ...
  submit() {
    const form = this.data.form;
    const hasGameType = form.gameTypeId || form.customGameType.trim();
    ...
    const payload = { ...form, maxPeople: Number(form.maxPeople), fee: Number(form.fee), startTime: `${form.date}T${form.time}:00` };
